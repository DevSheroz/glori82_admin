    ...
    <div className="max-h-[calc(100vh-220px)] overflow-y-auto">
      <table className="w-full text-sm">
        <thead className="sticky top-0 z-10">
          <tr className="border-b-2 border-(--color-border-base) bg-(--color-bg-subtle) shadow-[0_1px_3px_rgba(0,0,0,0.08)]">
            {columns.map((col) => (
              <th
                key={col.key}
                className="text-left text-xs font-semibold text-(--color-text-base) uppercase tracking-wider px-4 py-3"
        ...
        <tbody className="divide-y divide-(--color-border-base)">
          {data.map((row, i) => (
            <tr
              key={row.id ?? row.order_id ?? row.product_id ?? row.customer_id ?? i}
